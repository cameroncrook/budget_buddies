        ...
        const result = await pool.query(
            `SELECT
                ...
                (SELECT SUM(e.exp_cost)
                    ...
                    WHERE c.bg_id = $1 AND s.is_savings IS NOT TRUE
                    AND e.exp_date BETWEEN '2025-01-01' AND '2025-12-31'
    ...
    try {
        const result = await pool.query(
            `SELECT 
                c.cat_id, 
                c.cat_name,
                (SELECT SUM(sub_budget) FROM sub_category WHERE cat_id = c.cat_id AND is_savings IS NOT TRUE) AS category_budget,
